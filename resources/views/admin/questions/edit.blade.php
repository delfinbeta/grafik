    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <div class="p-4 bg-white overflow-hidden shadow-xl sm:rounded-lg">
        ...
        <form action="{{ route('admin.questions.update', $question) }}" method="post">
          ...
          <fieldset id="section_options" class="my-4 p-4 border border-gray-300 rounded-lg {{ (($question->type == 3) || ($question->type == 4)) ? '' : 'hidden' }}">
            ...
            <table id="table_options">
              @if ($question->options->isNotEmpty())
                ...
                <tr>
                  <td><button type="button" class="btn_delete p-2 rounded-md bg-red-500 hover:bg-red-600 text-white">- Eliminar</button></td>
                  <td>
                    <input type="text" name="options[]" value="{{ $option->title }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required />
                  </td>
                </tr>
                @endforeach
              @endif
